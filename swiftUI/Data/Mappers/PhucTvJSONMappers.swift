...
struct PhucTvHomeSectionDTO: Decodable, Sendable {
    let title: String
    let key: String
    let products: [PhucTvMovieCardDTO]
    let isCarousel: Bool
    ...
    init(from decoder: Decoder) throws {
        ...
        products = try container.decodeIfPresent([PhucTvMovieCardDTO].self, forKey: .products) ?? []
        isCarousel = try container.decodeBool(forKey: .isCarousel)
    }
}

struct PhucTvMovieCardDTO: Decodable, Sendable {
    let id: Int
    let name: String
...
struct PhucTvNavbarItemDTO: Decodable, Sendable {
    let id: Int
    let name: String
    let slug: String
    let items: [PhucTvNavbarItemDTO]
    ...
    init(from decoder: Decoder) throws {
        ...
        items = try container.decodeIfPresent([PhucTvNavbarItemDTO].self, forKey: .items) ?? []
        isExistChild = try container.decodeBool(forKey: .isExistChild)
    }
}

struct PhucTvPopupAdConfigDTO: Decodable, Sendable {
    let id: Int
    let name: String
    let type: String
...
struct PhucTvSimpleLabelDTO: Decodable, Sendable {
    let id: Int
    let name: String
    let link: String
...
struct PhucTvMovieEpisodeDTO: Decodable, Sendable {
    let id: Int
...
struct PhucTvMovieDetailDTO: Decodable, Sendable {
    let movie: PhucTvMovieCardDTO
    let relatedMovies: [PhucTvMovieCardDTO]
    let countries: [PhucTvSimpleLabelDTO]
    let categories: [PhucTvSimpleLabelDTO]
    let episodes: [PhucTvMovieEpisodeDTO]
    ...
    init(from decoder: Decoder) throws {
        let container = try decoder.container(keyedBy: CodingKeys.self)
        movie = try container.decode(PhucTvMovieCardDTO.self, forKey: .movie)
        relatedMovies = try container.decodeIfPresent([PhucTvMovieCardDTO].self, forKey: .relatedMovies) ?? []

        let movieContainer = try container.nestedContainer(keyedBy: MovieCodingKeys.self, forKey: .movie)
        countries = try movieContainer.decodeIfPresent([PhucTvSimpleLabelDTO].self, forKey: .countries) ?? []
        categories = try movieContainer.decodeIfPresent([PhucTvSimpleLabelDTO].self, forKey: .categories) ?? []
        episodes = try movieContainer.decodeIfPresent([PhucTvMovieEpisodeDTO].self, forKey: .episodes) ?? []
    }
...
struct PhucTvSearchFacetOptionDTO: Decodable, Sendable {
    let id: Int
    let name: String
    let slug: String
...
struct PhucTvSearchFilterDataDTO: Decodable, Sendable {
    let categories: [PhucTvSearchFacetOptionDTO]
    let countries: [PhucTvSearchFacetOptionDTO]
...
struct PhucTvSearchPaginationDTO: Decodable, Sendable {
    let pageIndex: Int
    let pageSize: Int
    let pageCount: Int
...
struct PhucTvSearchResultsDTO: Decodable, Sendable {
    let records: [PhucTvMovieCardDTO]
    let pagination: PhucTvSearchPaginationDTO
...
struct PhucTvPlayTrackDTO: Decodable, Sendable {
    let kind: String
    let file: String
    let label: String
    let isDefault: Bool
...
struct PhucTvPlaySourceDTO: Decodable, Sendable {
    let sourceId: Int
    ...
    init(from decoder: Decoder) throws {
        ...
        quality = try container.decodeString(forKey: .quality)
        tracks = try container.decodeIfPresent([PhucTvPlayTrackDTO].self, forKey: .tracks) ?? []
    }
}

extension PhucTvHomeSectionDTO {
    var domain: PhucTvHomeSection {
        PhucTvHomeSection(
            title: title,
            key: key,
...
extension PhucTvMovieCardDTO {
    var domain: PhucTvMovieCard {
        PhucTvMovieCard(
            id: id,
            name: name,
            otherName: otherName,
...
extension PhucTvNavbarItemDTO {
    var domain: PhucTvNavbarItem {
        PhucTvNavbarItem(
            id: id,
            name: name,
            slug: slug,
...
extension PhucTvPopupAdConfigDTO {
    var domain: PhucTvPopupAdConfig {
        PhucTvPopupAdConfig(
            id: id,
            name: name,
            type: type,
...
extension PhucTvSimpleLabelDTO {
    var domain: PhucTvSimpleLabel {
        PhucTvSimpleLabel(
            id: id,
            name: name,
            link: link,
...
extension PhucTvMovieEpisodeDTO {
    var domain: PhucTvMovieEpisode {
        PhucTvMovieEpisode(
            id: id,
...
extension PhucTvMovieDetailDTO {
    var domain: PhucTvMovieDetail {
        PhucTvMovieDetail(
            movie: movie.domain,
...
extension PhucTvSearchFacetOptionDTO {
    var domain: PhucTvSearchFacetOption {
        PhucTvSearchFacetOption(id: id, name: name, slug: slug)
    }
}

extension PhucTvSearchFilterDataDTO {
    var domain: PhucTvSearchFilterData {
...
extension PhucTvSearchPaginationDTO {
    var domain: PhucTvSearchPagination {
        PhucTvSearchPagination(
            pageIndex: pageIndex,
            pageSize: pageSize,
            pageCount: pageCount,
...
extension PhucTvSearchResultsDTO {
    var domain: PhucTvSearchResults {
        PhucTvSearchResults(
...
extension PhucTvPlayTrackDTO {
    var domain: PhucTvPlayTrack {
        PhucTvPlayTrack(kind: kind, file: file, label: label, isDefault: isDefault)
    }
}

extension PhucTvPlaySourceDTO {
    var domain: PhucTvPlaySource {
        PhucTvPlaySource(
            sourceId: sourceId,
